...
import axios from 'axios'; // --- NEW IMPORT for resend logic ---

// FIX APPLIED HERE: SERVER_URL ko sirf base server address par point kiya gaya hai.
// Yeh assume karta hai ki VITE_SERVER_URL set hai (e.g., http://localhost:3001)
...
    setErr("");
    setLoading(true);
    setNeedsVerificationEmail(null); // Reset verification state

    try {
      const payload = { email: email.trim(), password };
      // NOTE: The actual login API call is inside the imported 'login' function.
      // That is the most likely file (client/src/api/auth.js) that needs fixing now!
...
        setNeedsVerificationEmail(responseData.email); // Save email for resend
        msg = "Please verify your email to log in."; // Custom message for alert
      }
      // ------------------------------------------
...
      // Using axios directly for resend endpoint - (URL is correctly constructed)
      await axios.post(`${SERVER_URL}/api/auth/resend-verification`, { email: needsVerificationEmail });
...
          )}
          
          {/* Login Error Alert (If not verification error) */}
...
          {/* OAuth and Register Link (Intact) */}
